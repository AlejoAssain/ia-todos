    ...
    <article className="rounded-[28px] border border-white/10 bg-white/5 p-6 shadow-[0_20px_60px_rgba(5,2,16,0.45)] backdrop-blur-sm transition-transform duration-300 hover:-translate-y-1">
      ...
      <div className="mt-6 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <p className="text-sm text-violet-100/65">{task.steps.length} total</p>
        </div>
        ...
        {task.steps.length > 0 ? (
          <ul className="space-y-2">
            {task.steps.map((step, index) => (
              ...
              >
                <button
                  aria-label={step.completed ? 'Mark step as not done' : 'Mark step as done'}
                  ...
                  onClick={() => void handleToggleStep(step.id, !step.completed)}
                  type="button"
                >
                ...
                <p className="text-sm leading-6 text-violet-50/90">{step.content}</p>
              </li>
            ))}
          </ul>
        ) : (
